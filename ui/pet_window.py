...
class PetWindow(QWidget):
    ...
    def __init__(self, pet_manager, update_interval=50, settings=None, gateway_client=None):
        super().__init__()
        ...
        pet = pet_manager.get_current_pet()
        if not pet:
            raise RuntimeError("无当前助手，无法初始化 PetWindow")
        pet_config = pet_manager.get_current_pet_config()
        if pet_config:
        ...
        sprites_path = os.path.join(_root, pet_manager.pets_dir, pet.pet_name, "assets", "sprites")
        ...
        # macOS：Tool 窗口在应用失焦时会自动隐藏；设置此属性后助手窗口会一直停留在桌面上
        if is_macos():
        ...
        # 初始位置与固定尺寸（多显示器下避免 setGeometry 被 Qt 改写）；位置限制在当前屏幕内（避免 MAC 等“outside any known screen”）
        ...
        self.move(x, y)
        if pet and (x != pos.get("x", 100) or y != pos.get("y", 100)):
        ...
        logger.info(f"PetWindow 初始化完成")

    def _load_all_frames(self):
        pet = self.pet_manager.get_current_pet()
        ...
        for s in states:
            ...
            if not self._state_frames[s] and s != "idle":
                self._state_frames[s] = self._state_frames.get("idle") or self._state_frames.get("happy") or []
        if not self._state_frames.get("happy"):
            self._state_frames["happy"] = self._state_frames.get("idle") or []
    ...
    def showEvent(self, event):
        ...
        # Mac：首次显示后延迟校正位置，避免保存的坐标在换屏/分辨率下跑出屏幕
    ...
    def _setup_movement(self):
        pet = self.pet_manager.get_current_pet()
        pet_config = self.pet_manager.get_current_pet_config()
    ...
    def on_user_activity(self):
        ...
        cfg = self.pet_manager.get_current_pet_config()
    ...
    def _apply_state_target(self, target, apply_anim=True):
        """统一状态切换入口：更新 _current_state、hold、动画；仅在状态变化时写 pet.set('state')。"""
    ...
    def open_chat(self):
        ...
        pet = self.pet_manager.get_current_pet()
        cfg = self.pet_manager.get_current_pet_config()
        name = (pet.get("name") or pet.pet_name) if pet else t("pet_default_name")
    ...
    def open_clear_cache(self):
        ...
        pet = self.pet_manager.get_current_pet()
        bot_id = (pet.get("bot_id") or getattr(pet, "pet_name", None) or "bot00001") if pet else "bot00001"
        def on_create(w):
            w.bot_id = bot_id
    ...
    def _on_auto_interaction_tick(self):
        ...
        if self._last_auto_run_ts > 0:
            ...
            if self._auto_interaction_start_ts <= 0 or (now - self._auto_interaction_start_ts) < interval_sec:
                return
        prompt = extract_random_skill(self.pet_manager.get_current_pet())
        if not prompt:
            return
    ...
    def _switch_to_walk(self):
        ...
        if self._bubble_done_walk_timer:
            self._bubble_done_walk_timer.stop()
            self._bubble_done_walk_timer = None
        happy_sec = float(self.pet_manager.get_current_pet_config().get_timing("happy_after_action_sec", 60)) if self.pet_manager.get_current_pet_config() else 60
    ...
    def set_forced_state(self, state):
        """用户自定义助手状态，config.timings.forced_state_duration_sec 内强制生效，之后恢复代码逻辑。None 表示取消强制；取消时刷新自动状态起点，避免立即切到 walk。"""
        cfg = self.pet_manager.get_current_pet_config()
    ...
    def _switch_robot(self, pet_id):
        ...
        if pet_id == self.pet_manager.current_pet_name:
            return
        if not self.pet_manager.switch_pet(pet_id):
            return
        if self.settings is not None:
            pet = self.pet_manager.get_current_pet()
            bot_id = pet.get("bot_id", "bot00001") if pet else "bot00001"
            self.settings.set("current_pet", bot_id)
            self.settings.load()
            self.settings.save()
        self._reload_for_current_pet()

    def _reload_for_current_pet(self):
        ...
        sprites_path = os.path.join(_root, self.pet_manager.pets_dir, pet.pet_name, "assets", "sprites")
        self._sprites_path = sprites_path if os.path.isdir(sprites_path) else None
        self._load_all_frames()
        pos = pet.get_position()
        self.setFixedSize(self._display_size[0], self._display_size[1])
        ...
        logger.info(f"已切换机器人并重载: {pet.pet_name}")
    ...
    def build_pet_context_menu(self, menu):
        ...
        for pid in self.pet_manager.list_pets():
            pd = self.pet_manager.pets.get(pid)
            display_name = (pd.get("name") or pid) if pd else pid
            label = display_name + t("current_suffix") if pid == self.pet_manager.current_pet_name else display_name
    ...
    def _show_context_menu(self, global_pos):
        ...
        menu = QMenu(self)
        self.build_pet_context_menu(menu)
