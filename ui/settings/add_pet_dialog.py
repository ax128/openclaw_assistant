...
from core.pet_data import DEFAULT_CONFIG, DEFAULT_STATE_TO_SPRITE_FOLDER
...
def _read_next_bot_seq(pets_dir: str) -> int:
    """读取 next_bot_seq.json 中的 next 值；文件不存在时根据现有助手取 max+1 并写入。空缺不补。"""
    path = os.path.join(pets_dir, NEXT_BOT_SEQ_FILE)
    ...
    if os.path.isdir(pets_dir):
        for name in os.listdir(pets_dir):
            if name == NEXT_BOT_SEQ_FILE:
                continue
            data_file = os.path.join(pets_dir, name, "data.json")
...
def get_next_bot_id(pets_dir: str) -> str:
    """返回下一个将使用的 bot_id（格式 bot00001），不递增。用于预览。"""
    return "bot%05d" % _read_next_bot_seq(pets_dir)


def consume_next_bot_id(pets_dir: str) -> None:
    """新增助手成功后调用：将 next_bot_seq 递增，保证空缺不补。"""
    path = os.path.join(pets_dir, NEXT_BOT_SEQ_FILE)
    n = _read_next_bot_seq(pets_dir)
    try:
        with open(path, "w", encoding="utf-8") as f:
...
def _next_bot_id(pets_dir: str) -> str:
    """返回下一个将使用的 bot_id（同 get_next_bot_id，兼容旧调用）。"""
    return get_next_bot_id(pets_dir)
...
def _validate_sprite_files(file_paths: list) -> tuple[bool, str]:
    """
    校验选中的图片：必须为 1.png, 2.png, ... N.png 连续，N<=30。
    返回 (ok, error_message)。
    """
    if not file_paths:
        return True, ""
...
class AddPetDialog(QDialog):
    ...
    def __init__(self, pets_dir: str, parent=None):
        super().__init__(parent)
        self.pets_dir = os.path.normpath(pets_dir)
        self._state_files = {}  # state_key -> [(src_path, target_name), ...]
    ...
    def _update_bot_id_preview(self):
        """预览下一 bot_id（仅展示用，提交时再算一次）。"""
        self.bot_id_label.setText(_next_bot_id(self.pets_dir))
    ...
    def _validate_form(self) -> str:
        ...
        pet_path = os.path.join(self.pets_dir, folder)
        if os.path.isdir(pet_path) and os.path.isfile(os.path.join(pet_path, "data.json")):
    ...
    def _on_ok(self):
        ...
        bot_id = get_next_bot_id(self.pets_dir)

        pet_root = os.path.join(self.pets_dir, folder)
        assets = os.path.join(pet_root, "assets")
        ...
        try:
            ...
            # 仅对用户选择了图片的表情创建文件夹并复制；未选择的不创建，避免空目录
            ...
            data_path = os.path.join(pet_root, "data.json")
            with open(data_path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            consume_next_bot_id(self.pets_dir)
