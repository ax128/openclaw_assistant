...
from ui.settings.add_pet_dialog import (
    SPRITE_STATE_KEYS,
...
def _list_pet_folders(pets_dir: str) -> list:
...
class EditPetDialog(QDialog):
    ...
    def __init__(self, pets_dir: str, parent=None):
        ...
        self._current_folder = None  # 当前选中的助手文件夹名
        ...
        folders = _list_pet_folders(self.pets_dir)
        for f in folders:
    ...
    def _on_pet_selected(self):
        ...
        if not self._current_folder:
            return
        data_path = os.path.join(self.pets_dir, self._current_folder, "data.json")
    ...
    def _on_save(self):
        ...
        pet_root = os.path.join(self.pets_dir, self._current_folder)
        ...
        try:
            ...
            # 仅对用户选择了图片的表情文件夹写入；未选择的不修改，避免覆盖
            overwrite_folders = []
